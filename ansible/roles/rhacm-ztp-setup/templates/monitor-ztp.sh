...
echo "date,initialized,booted,discovered,provisioning,install_failed,install_completed,managed,policy_applying,policy_timedout,policy_compliant" > ${csv_file} | tee -a ${log_file}
...
while true; do
  ...
  initialized=$(echo "${aci_completed}" | grep -c sno | tr -d " ")
  ...
  echo "${cur_dt},${initialized},${booted},${discovered},${provisioning},${install_failed},${install_completed},${managed},${policy_applying},${policy_timedout},${policy_compliant}" >> ${csv_file} | tee -a ${log_file}
